...
Nothing in the active pipeline imports this module anymore. Its role (a
per-dataset STUDY CONTEXT block + swap_integration_closers()) is now played
by prompt_registry.DATASET_BLOCKS and the _fill() build-time substitution
inside prompt_registry.get_prompts / get_charmaz_recursion_prompts /
get_straussian_slot_prompts. Kept on disk for historical reference / diffing
only -- in particular, this file documents the original (longer, more
hedged) semeval STUDY CONTEXT wording that was later shortened and made more
direct per user feedback. Do not edit this file expecting it to affect a
real run.
