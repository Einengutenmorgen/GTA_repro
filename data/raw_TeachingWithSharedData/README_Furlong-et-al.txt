**Title**
Data for: Teaching with Shared Data for Learning Qualitative Data Analysis: A Multi-Sited Case Study of Instructor and Student Experiences

**Suggested Citation**
Please cite any data used from this data project as follows: 

Furlong, Darcy E.; Romero, Anna; Helström, Kirstin; Lester, Jessica Nina; Karcher, Sebastian. 2024. "Data for: Teaching with Shared Data for Learning Qualitative Data Analysis: A Multi-Sited Case Study of Instructor and Student Experiences". Qualitative Data Repository. https://doi.org/10.5064/F6XZV8BZ. QDR Main Collection. 

**Description**
 Project Overview
This study investigates the teaching methods used by two instructors of qualitative research methods at two different academic institutions to integrate data analysis instruction into their curricula. Situated within the theoretical frameworks of active learning, this work draws upon contemporary scholarship that underscores the importance of student engagement through hands-on analytical activities. We specifically focus on the concept of “shared data” – defined as data collected or archived by others, such as datasets sourced from online repositories or anonymized collections prepared by educators for teaching demonstrations – and examine how these resources are utilized in two instructional settings. Employing a qualitative case-study design, this project analyzes the pedagogical approaches in both an introductory and an advanced qualitative methods course. We show how each instructor leverages shared data differently to facilitate skill-building in qualitative data analysis, specifically considering both the introductory and advanced contexts. By juxtaposing these instructional practices, this study offers insights into the nuanced applications of shared data in qualitative research education and highlights the role of data type, course level, and instructional environment in shaping students’ analytical competencies.

**File Access**
Documentation freely accessible under the Creative Commons Attribution-Share Alike 4.0 license.
Data files accessible without restrictions for all registered QDR users under our Standard Download Agreement.


**Included Files**
This collection contains the following files:

Furlong-et-al_Codebook.pdf
Furlong-et-al_DataNarrative.pdf
Furlong-et-al_InterviewSiteA_Instructor1.pdf
Furlong-et-al_InterviewSiteA_Instructor2.pdf
Furlong-et-al_InterviewSiteA_Student1.pdf
Furlong-et-al_InterviewSiteB_Instructor1.pdf
Furlong-et-al_InterviewSiteB_Instructor2.pdf
Furlong-et-al_InterviewSiteB_Student1.pdf
Furlong-et-al_InterviewSiteB_Student2.pdf
Furlong-et-al_InterviewSiteB_Student3.pdf
Furlong-et-al_InterviewSiteB_Student4.pdf

**Data Processing Note**

The data were processed according to the Qualitative Data Repository's curation policy. All files were checked for integrity. Files were checked to make sure they match documentation and consistent file names were applied. PDF files were converted to PDF/A using Adobe Acrobat Pro DC Version 2022.001.20117, XMP tags edited using pikepdf, and enhanced for WCAG accessibility compliance.

[edit data processing note according to processing steps, note everything we did with the data -- e.g., add notes on de-identification (2.3.1.), copyright (2.3.2.), and other file conversions (2.4.) -- and update software versions as necessary]

*****

These data were obtained from the Qualitative Data Repository (QDR - http://qdr.syr.edu), a dedicated archive for storing and sharing digital data (and accompanying documentation) generated or collected through qualitative and multi-method research in the social sciences. QDR is supported by the National Science Foundation. Please consult QDR's General Terms and Conditions of Use (https://qdr.syr.edu/termsandconditions). 
